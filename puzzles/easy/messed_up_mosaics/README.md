...
They were cheap, but they all came with the same issue:
**one single tile is not the good one**.

Can you help me locate the wrong tile in each mosaic?

Each mosaic is made of `N` \* `N` **tiles**.

Given a `pattern` of tiles that repeats to form a `mosaic` and the `mosaic`
itself, you have to find the `coordinates` of the tile that doesn't fit the
`pattern`.
...
- **Line 1**: An int `N` for the length (in tiles) of the side of the square
  `mosaic`
...
- A single line containing the coordinates of the wrong tile, formatted as
  `(x,y)`
